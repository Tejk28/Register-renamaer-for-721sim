...
private:
	...
	/////////////////////////////////////////////////////////////////////
        ...
        uint64_t ActiveList_size;
        unsigned int PhyRegs_num;
        unsigned int logRegs_num;
        unsigned int Unresolved_Branches_num;
	uint64_t commit_counter;       
	...
	/////////////////////////////////////////////////////////////////////
        uint64_t *FreeList;
        bool HeadPhaseFreeList;
	int HeadFreeList;
        bool TailPhaseFreeList;
        int TailFreeList;
	...
	bool HeadPhaseActiveList;
 	int HeadActiveList;
 	bool TailPhaseActiveList;
	int TailActiveList;
	...
	// 3. checkpointed GBM
	/////////////////////////////////////////////////////////////////////
	struct Branch_Checkpoints{
		uint64_t *SMT;
		int Head_FreeList;
		uint64_t GBM;
 		//unsigned int Branch_count;
 		bool HeadPhase_FreeList;
	};
	Branch_Checkpoints **BrCheckPoint;
	/////////////////////////////////////////////////////////////////////
	// Private functions.
	// e.g., a generic function to copy state from one map to another.
	/////////////////////////////////////////////////////////////////////
	void map_copy(unsigned long* source, unsigned long* Dest, unsigned int size )
	{
	...
	void print_al()
	 ...
	 {
           printf("HEAD = %d, TAIL=%d, H_PHASE=%d, T_PHASE=%d, SIZE=%d\n",HeadActiveList,TailActiveList,HeadPhaseActiveList,TailPhaseActiveList,ActiveList_size);
           for (int i =HeadActiveList; i<TailActiveList; i++) 
		{
             printf("%d: Valid_dest:%d, log_reg:%d, phy_reg:%d, comp:%d, excep:%d, ld_viol:%d, br_m:%d, val_m:%d, ld_fl:%d, str_fl:%d, br_fl:%d, amo_fl:%d, csr_fl:%d, pc:%d\n",i,ActiveList[i].has_Destination, ActiveList[i].log_Destination,ActiveList[i].phys_Destination,ActiveList[i].completed,ActiveList[i].exception,ActiveList[i].load_violation,ActiveList[i].branch_misprediction,ActiveList[i].value_misprediction,ActiveList[i].isLoad,ActiveList[i].isStore,ActiveList[i].isBranch,ActiveList[i].is_amo,ActiveList[i].is_csr,ActiveList[i].PC);
          	 }
         }

	void print_RMT()
	{
	cout<<"Printing RMT values"<<endl;
	...
	void print_AMT()
	{
	cout<<"Printing AMT values"<<endl;
	for(int i=0; i< logRegs_num; i++)
		{	
		printf("AMT[%d] = %d \t",i, AMT[i]);
		}
	}
	...
	void print_fl()
	{
           ...
           printf("HEAD_ = %d, TAIL=%d, H_PHASE=%d, T_PHASE=%d, SIZE=%d\n",HeadActiveList,TailActiveList,HeadPhaseActiveList,TailPhaseActiveList,ActiveList_size);
           for (int i =HeadActiveList; i<TailActiveList; i++) 
		{
             printf("%d: Valid_dest:%d, log_reg:%d, phy_reg:%d, comp:%d, excep:%d, ld_viol:%d, br_m:%d, val_m:%d, ld_fl:%d, str_fl:%d, br_fl:%d, amo_fl:%d, csr_fl:%d, pc:%d\n",i,ActiveList[i].has_Destination, ActiveList[i].log_Destination,ActiveList[i].phys_Destination,ActiveList[i].completed,ActiveList[i].exception,ActiveList[i].load_violation,ActiveList[i].branch_misprediction,ActiveList[i].value_misprediction,ActiveList[i].isLoad,ActiveList[i].isStore,ActiveList[i].isBranch,ActiveList[i].is_amo,ActiveList[i].is_csr,ActiveList[i].PC);
          	 }
	...
	void print_GBM()
	{
		cout<<"Debug: Printing GBM"<<endl;
		uint64_t tempGBM = GBM;
		uint64_t mask =0;
		mask |= 1ULL << (Unresolved_Branches_num -1);
		
		for (int i=0; i< Unresolved_Branches_num; i++)
		{
			bool bit = tempGBM & mask;
			cout<<bit;
			mask = mask >>1;
		}
	cout<<endl;	
...
/*
	void print_SMT()
	{
		cout<<"Debug:Printing SMT values"<<endl;	
	
			for(int j=0; j<logRegs_num; j++)
			{
			printf("\nSMT[%d] = %d",j,BrCheckPoint[j].SMT[j]);
			}
			cout<<endl;
